...
func (t PromptTemplateApi) InitDashboardPrompt() {
	...
	if count == 0 {
		...
		t.upgradeDashboardPromptIfLegacy()
	}
}

const legacyDashboardPromptPrefix = "你是一位专注于趋势交易的"

// upgradeDashboardPromptIfLegacy 将仍为旧版内置默认文案的仪表盘模板升级为本期投顾大师协议（不覆盖用户完全自定义内容）
func (t PromptTemplateApi) upgradeDashboardPromptIfLegacy() {
	...
	if strings.HasPrefix(strings.TrimSpace(p.Content), legacyDashboardPromptPrefix) {
		_ = db.Dao.Model(&models.PromptTemplate{}).Where("id = ?", p.ID).Update("content", DefaultDashboardPrompt).Error
	}
}
